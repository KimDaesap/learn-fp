...
class FunctorOps[A, F[_]](a: F[A])(implicit functor: Functor[F]) {
  ...
  def map[B](fx: A => B): F[B] =
    functor.fmap(a)(fx) // to be compatible with for
}
...
object FunctorOps {
  ...
  implicit def toFunctorOps[A, F[_]](f: F[A])(
      implicit functor: Functor[F]
  ): FunctorOps[A, F] = new FunctorOps(f)

  implicit def fxToFunctorOps[A, B](fx: A => B): FxFunctorOps[A, B] =
    new FxFunctorOps[A, B](fx)
}
