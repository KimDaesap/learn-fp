...
object DisjunctionInstance {
  ...
  implicit def eitherInstance[L] =
    new Functor[({ type E[A] = Disjunction[L, A] })#E] {
      override def fmap[A, B](a: Disjunction[L, A])(
          fx: A => B
      ): Disjunction[L, B] = a match {
        case LeftDisjunction(lv) => LeftDisjunction(lv)
  ...
  implicit def toFxOps[A, R](fx: A => R): DisjunctionFxOps[A, R] =
    new DisjunctionFxOps(fx)
}
