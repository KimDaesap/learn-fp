...
object State {
  def put[S](ns: S): State[S, Unit] = State(_ => (ns, Unit))
...
object StateInstance {
  ...
  implicit def stateInstance[S] = new Functor[({ type E[X] = State[S, X] })#E] {
    override def fmap[A, B](a: State[S, A])(fx: A => B): State[S, B] = State(s => (a.exec(s), fx(a.eval(s))))
  }

  implicit def toFunctorOps[S, A, State[S, A]](
      f: State[S, A]
